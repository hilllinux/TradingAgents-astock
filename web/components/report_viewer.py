...
def _refresh_missing_data_state(
    final_state: dict[str, Any],
    ticker: str,
    trade_date: str,
) -> dict[str, Any]:
    live_tasks = get_missing_tasks(ticker, trade_date, active_only=False)
    if not live_tasks:
        return dict(final_state)

    saved_tasks = _active_missing_tasks(final_state, ticker, trade_date)
    live_by_id = {task["id"]: task for task in live_tasks if task.get("id")}
    pending = [
        task for task in saved_tasks
        if task.get("id") not in live_by_id
    ]
    pending.extend(task for task in live_tasks if task.get("status", "active") == "active")
    resolved_saved = any(
        live_by_id.get(task.get("id"), {}).get("status") in {"resolved", "consumed"}
        for task in saved_tasks
    )
    requires_reanalysis = (
        bool(final_state.get("missing_data_requires_reanalysis"))
        or resolved_saved
        or any(
            task.get("status") == "resolved" and task.get("resolved_output_path")
            for task in live_tasks
        )
    )
    complete = final_state.get("missing_data_complete")
    if pending:
        complete = False
    elif resolved_saved:
        complete = True
    return {
        **final_state,
        "missing_data_tasks": pending,
        "missing_data_complete": complete,
        "missing_data_requires_reanalysis": requires_reanalysis,
    }
...
def _render_missing_data_panel(
    ...
    final_state = _refresh_missing_data_state(final_state, ticker, trade_date)
    ...
    if count == 0:
        ...
        elif final_state.get("missing_data_complete") is True:
    ...
    with st.expander(f"查看缺失/失败取数项（{count}）", expanded=False):
        ...
        if st.button("重新取数这些缺失项", key=retry_key, type="primary"):
...
def render_report(
    ...
    st.caption("缺失清单仅覆盖已调用接口，不代表所有财务字段齐全，也不代表质量审核通过。")
    ...
    with st.expander("数据质量审核与限制", expanded=False):
        audit = final_state.get("data_quality_summary") or "未保存或未提供质量审核结果，不得视为审核通过。"
        st.markdown(_display_report_text(audit, ticker, final_state))
