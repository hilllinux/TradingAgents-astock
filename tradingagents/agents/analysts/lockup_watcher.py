...
def create_lockup_watcher(llm):
    ...
    def lockup_watcher_node(state):
        ...
        system_message = (
            ...
            "\n- **减持预披露**：核对公告中的主体、数量上限、方式和执行窗口，区分拟减持、实施中、已完成及终止。预案不是实际卖出，也不是确定性价格利空。"
            ...
            "\n5. 未来 3 个月潜在减持风险评估"
            + "\n合规结论必须依据分析基准日有效规则及公告原文，核对一致行动人适用范围、股份来源和限制条件；不能仅因不是控股股东本人或比例较小就认定合规，缺少原文则待核验。"
            + "\n执行起止日优先引用公告；只有具备适用规则和交易日历时才计算并标明计算值，缺少条件不估写十月初等执行日期。金额上限是情景测算，不等于实际抛压。"
