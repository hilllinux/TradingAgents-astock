...
@pytest.mark.parametrize("content", [
    "[数据缺失] 新浪读取失败: Timeout；不是公司未披露。",
    "已取得资产负债表\n[数据缺失] HiThink读取失败: Timeout；不替换或补零。",
    "[数据缺失：ROE]",
    "Baidu PAE 风控拦截（hit risk），002044 的概念板块本次取不到。",
    None,
    "",
    "   \n",
    [],
...
def test_local_source_gaps_and_empty_outputs_are_tracked(content, missing_index):
    task = missing_data.record_tool_result(
        ticker="002044", trade_date="2026-09-21", stage="fundamentals",
        tool_name="get_balance_sheet", args={}, content=content,
    )
    assert task is not None
...
@pytest.mark.parametrize("output", [None, "", "[数据缺失] 尚未返回可核验的记录。"])
def test_retry_cannot_resolve_empty_or_unverified_data(output, missing_index, monkeypatch):
    missing_data.record_tool_result(
        ticker="002044", trade_date="2026-09-21", stage="fundamentals",
        tool_name="get_balance_sheet", args={}, content="[数据缺失: 资产负债表]",
    )
    tool = MagicMock()
    tool.invoke.return_value = output
    monkeypatch.setattr(missing_data, "_tool_registry", lambda: {"get_balance_sheet": tool})
    monkeypatch.setattr(missing_data, "sync_missing_snapshot_to_analysis_log", lambda *args, **kwargs: None)

    result = missing_data.retry_missing_tasks("002044", "2026-09-21")

    assert result["resolved_count"] == 0
    assert result["remaining_count"] == 1
    assert not list(missing_data._MISSING_DATA_CACHE_DIR.glob("*.txt"))
